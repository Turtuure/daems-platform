...
final class ReduceMemberFeeInvoiceTest extends TestCase
{
    ...
    public function test_admin_reduces_invoice(): void
    {
        ...
        $useCase = new ReduceMemberFeeInvoice($invoices, $audit, new FrozenClock(new DateTimeImmutable('2026-09-20')));
        $useCase->handle(new ReduceMemberFeeInvoiceInput(
            actor:          $this->actor(),
            invoiceId:      $invoice->id(),
            newAmountCents: 2500,
            reason:         'Sosiaalinen alennus',
        ));

        $loaded = $invoices->findById($invoice->id());
        $this->assertSame(MemberFeeInvoiceStatus::Reduced, $loaded?->status());
        $this->assertSame(2500, $loaded?->amountCents());
        $this->assertSame(5000, $loaded?->originalAmountCents());
    ...
    public function test_non_admin_rejected(): void
    {
        ...
        $useCase = new ReduceMemberFeeInvoice($invoices, $audit, new FrozenClock(new DateTimeImmutable()));
    ...
    public function test_invalid_amount_above_current_rejected(): void
    {
        $invoices = new InMemoryMemberFeeInvoiceRepository();
        $audit = new InMemoryFeeInvoiceAuditRepository();
        $invoice = $this->seedInvoice();
