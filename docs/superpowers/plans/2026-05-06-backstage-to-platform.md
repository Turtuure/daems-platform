...
| 2026-05-06 | F+ | fix | society:2047efb | **Architecture correction after user feedback.** Original plan used cross-host 301 redirect from `daem-society.local/backstage` to `daems-platform.local/backstage` — broke the tenant-frontend session (re-login required). Replaced with same-host filesystem delegation: `daem-society/public/index.php` `require`s platform's `backstage.php` logic, serving backstage at `daems.local/backstage` while reusing the existing `$_SESSION['user']`. Static assets (`/backstage/assets/*`, `/backstage/pages/*` CSS/JS, `/modules-shared/*`, `/modules/<n>/assets/*`) served from platform filesystem. Platform's `daems-platform.local/backstage` retained as direct-access fallback. Sahegroup multi-tenant requires a follow-up: ApiClient must send `X-Daems-Forwarded-Host` so platform's `TenantContextMiddleware` resolves the tenant from the original frontend host, not from `daems-platform.local`. |
